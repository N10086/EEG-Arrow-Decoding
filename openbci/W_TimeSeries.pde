...
class W_timeSeries extends Widget {
    ...
    void draw() {
        ...
        // ===== 数据流停止时自动保存图表 =====
        boolean isStreaming = (currentBoard != null && currentBoard.isStreaming());
        if (wasStreaming && !isStreaming) {
            // Streaming 刚停止 → 保存图表快照
            saveStimulusChartImage();
        }
        wasStreaming = isStreaming;
    }

    // ---- 保存标记标注后的图表快照 ----
    private void saveStimulusChartImage() {
        if (w <= 0 || h <= 0) return;
        try {
            // 捕获 widget 区域的像素
            PImage chartImg = get(x, y, w, h);
            // 生成文件名：录制目录下
            String dir = directoryManager.getGuiDataPath() + "Screenshots"
                + System.getProperty("file.separator");
            new File(dir).mkdirs();
            String fname = "stimulus_chart_"
                + nf(year(), 4) + "-" + nf(month(), 2) + "-" + nf(day(), 2)
                + "_" + nf(hour(), 2) + "-" + nf(minute(), 2) + "-" + nf(second(), 2)
                + ".png";
            chartImg.save(dir + fname);
            outputInfo("刺激标注图表已保存 → " + dir + fname);
            println("W_timeSeries: chart saved -> " + dir + fname);
        } catch (Exception e) {
            println("W_timeSeries: save chart failed - " + e.getMessage());
        }
    }
...
class ChannelBar {
    ...
    public void draw(boolean hardwareSettingsAreOpen) {
        ...
        // ===== 在画布像素坐标上绘制刺激标记竖线（叠加在 EEG 曲线上方） =====
        long nowMs = System.currentTimeMillis();
        pushStyle();
        for (StimulusEvent evt : stimulusEvents) {
            float secAgo = (nowMs - evt.wallTimeMs) / 1000.0f;
            if (secAgo >= 0 && secAgo < numSeconds) {
                float markerX = -secAgo;
                // data → pixel 转换
                float[] plotPos = plot.getPos();
                float[] layerDim = plot.getMainLayer().getDim();
                float px_rel = plot.getMainLayer().valueToXPlot(markerX);
                float px_abs = plotPos[0] + px_rel;
                float py_top = plotPos[1];
                float py_bot = py_top + layerDim[1];
                // 红色实线
                stroke(255, 50, 50);
                strokeWeight(2);
                line(px_abs, py_top, px_abs, py_bot);
                // 标记编号标签
                fill(255, 50, 50);
                noStroke();
                textSize(10);
                textAlign(LEFT, BOTTOM);
                String label = evt.name.isEmpty() ? ("M#" + evt.markerNumber) : evt.name;
                text(label, px_abs + 2, py_top - 2);
            }
        }
        popStyle();
