...
class TopNav {
    ...
    public Button videoImportButton;
    ...
    void initSecondaryNav() {
        ...
        // 视频导入/播放按钮（紧跟 Smoothing 按钮之后）
        if (videoImportButton == null) {
            int btnPosX = PAD_3;
            if (smoothingButton != null) {
                btnPosX = (int)smoothingButton.getPosition()[0] + smoothingButton.getWidth() + PAD_3;
            } else {
                btnPosX = (int)filtersButton.getPosition()[0] + filtersButton.getWidth() + PAD_3;
            }
            createVideoImportButton("刺激实验", btnPosX, SUBNAV_BUT_Y, 130, SUBNAV_BUT_H,
                                    h4, 14, SUBNAV_LIGHTBLUE, WHITE);
        }
    ...
    void update() {
        ...
        // 刷新视频导入按钮文本
        if (videoImportButton != null && stimController != null) {
    ...
    void draw() {
        ...
        if (videoImportButton != null) {
            videoImportButton.setVisible(isSession);
        }
    ...
    void screenHasBeenResized(int _x, int _y) {
        ...
        if (systemMode == SYSTEMMODE_POSTINIT) {
            ...
            // 重新定位 Smoothing 和视频导入按钮
            if (smoothingButton != null) {
                int smoothPos = (int)filtersButton.getPosition()[0] + filtersButton.getWidth() + PAD_3;
                smoothingButton.setPosition(smoothPos, SUBNAV_BUT_Y);
                if (videoImportButton != null) {
                    videoImportButton.setPosition(smoothPos + smoothingButton.getWidth() + PAD_3, SUBNAV_BUT_Y);
                }
            } else {
                if (videoImportButton != null) {
                    videoImportButton.setPosition((int)filtersButton.getPosition()[0] + filtersButton.getWidth() + PAD_3, SUBNAV_BUT_Y);
                }
            }
    ...
    /** 更新刺激实验按钮文本 */
    public void updateVideoButtonText() {
        if (videoImportButton == null) return;
        videoImportButton.getCaptionLabel().setText("刺激实验");
        videoImportButton.setColorBackground(SUBNAV_LIGHTBLUE);
    }
    ...
    // ---- 刺激实验控制台按钮 ----
    private void createVideoImportButton(String text, int _x, int _y, int _w, int _h, PFont font, int _fontSize, final color _bg, color _textColor) {
        videoImportButton = createTNButton("videoImportButton", text, _x, _y, _w, _h, font, _fontSize, _bg, _textColor);
        videoImportButton.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                if (stimLauncher != null) {
                    stimLauncher.open();
                }
            }
        });
        videoImportButton.setDescription("打开刺激实验控制台：视频标记或回放测试");
    }
