...
class PacketRecord {
    ...
    // 简单累加方法 - 避免 Processing 4.x 泛型解析问题
    public void addLost(int lost) { numLost += lost; }
    public void addReceived(int received) { numReceived += received; }
    public void addLostReceived(int lost, int received) {
        numLost += lost;
...
class PacketLossTracker {
    ...
    // Processing 4.x compat: avoid generic type PacketRecord in method signatures
    public List getAllPacketRecordsForLast(int milliseconds) {
        return packetRecords.getLastData(milliseconds);
    }

    public PacketRecord getCumulativePacketRecordForLast(int milliseconds) {
        List allRecords = getAllPacketRecordsForLast(milliseconds);
        PacketRecord result = new PacketRecord(0, 0);
        for (int i = 0; i < allRecords.size(); i++) {
            PacketRecord r = (PacketRecord) allRecords.get(i);
            result.addLostReceived(r.numLost, r.numReceived);
        }
        return result;
    }
