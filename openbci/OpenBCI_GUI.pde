...
import controlP5.Button; // resolves ambiguity with java.awt.Button when compiling with javac
import processing.data.Table; // resolves ambiguity with jogamp and mordant Table
import java.awt.Graphics; // resolves ambiguity with skia Graphics
import java.util.Date; // resolves ambiguity with ant Date
...
// ---- 视频刺激控制器 ----
StimulusController stimController;
StimulusLauncher stimLauncher;
...
void delayedSetup() {
    ...
    // 初始化视频刺激控制器
    stimController = new StimulusController();
    stimLauncher = new StimulusLauncher();
    println("StimulusController: Video stimulus controller initialized.");
    println("Use the '刺激实验' button in the top nav to open the control panel.");
...
void startRunning() {
    ...
    if (currentBoard.isStreaming()) {
        ...
        // 同步启动视频刺激（与 EEG 数据流同时播放）
        if (stimController != null && stimController.hasVideo() && !stimController.isActive()) {
            stimController.play();
        }
...
void systemUpdate() { // for updating data values and variables
    ...
    // 更新视频刺激播放状态（先于 dataLogger 写入，确保标记值最新）
    if (stimController != null) {
        stimController.update();
    }
