abstract class DataSourcePlayback implements DataSource, FileBoard  {
    ...
    protected boolean parseExgData(String[] lines) {
        ...
        // ---- 检测 Stimulus Marker 列的位置 ----
        int stimMarkerCol = -1;
        if (dataStart > 0 && dataStart <= lines.length) {
            String headerLine = lines[dataStart - 1];
            String[] colNames = headerLine.split(",");
            for (int i = 0; i < colNames.length; i++) {
                if (colNames[i].trim().equals("Stimulus Marker")) {
                    stimMarkerCol = i;
                    break;
                }
            }
        ...
        for (int iData=0; iData<dataLength; iData++) {
            ...
            // ---- 提取 Stimulus Marker 值（在裁剪前读取） ----
            if (stimMarkerCol >= 0 && stimMarkerCol < valStrs.length) {
                try {
                    double markerVal = Double.parseDouble(valStrs[stimMarkerCol].trim());
                    if (markerVal >= 2.0) {
                        // 解码标记编号：2.0005 → #5
                        int markerNum = (int) Math.round((markerVal - 2.0) * 10000);
                        if (markerNum < 1) markerNum = (int)markerVal;
                        stimulusEvents.add(new StimulusEvent(
                            System.currentTimeMillis(), markerNum, markerVal,
                            markerNum >= 1 && markerNum <= 4
                                ? new String[]{"A", "B", "C", "D"}[markerNum - 1]
                                : ""));
                    }
                } catch (Exception e) { /* 跳过无法解析的标记值 */ }
            }

            // VEP/AEP: 如果文件包含 Stimulus Marker 列，裁剪多余列
            if (underlyingBoard != null && numChannelsExg > underlyingBoard.getTotalChannelCount()) {
                numChannelsExg = underlyingBoard.getTotalChannelCount();
            }
